...
@section('title', __('نسيت كلمة المرور'))
...
<div class="auth-page-wrapper">
    <div class="auth-card">
        ...
        <h2 class="auth-title">{{ __('نسيت كلمة المرور؟') }}</h2>
        <p class="auth-subtitle">{{ __('أدخل بريدك الإلكتروني وسنرسل لك رابطاً لإعادة تعيين كلمة المرور.') }}</p>
        ...
        <form method="POST" action="{{ route('password.email') }}" id="forgotPasswordForm">
            ...
            <button type="submit" class="btn-auth-submit" id="submitBtn">
                <span>{{ __('إرسال رابط استعادة كلمة المرور') }}</span>
                <i class="fas fa-paper-plane"></i>
            </button>
        </form>

        <div class="auth-footer-link">
            <a href="{{ route('login') }}"><i class="fas fa-arrow-right me-1"></i> {{ __('العودة لتسجيل الدخول') }}</a>
        </div>
    </div>
</div>
@endsection
