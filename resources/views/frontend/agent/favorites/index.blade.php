...
<div class="dash-header-row mb-4">
    ...
    <p class="text-muted" style="font-size: 0.9rem;">{{ __('Quick access to the trips you are interested in.') }}</p>
</div>
...
@if($favorites->count() > 0)
    <div class="favorites-grid">
        @foreach($favorites as $favorite)
            ...
            @endphp
            @if($trip)
                <div class="favorite-card" id="fav-container-{{ $trip->id }}">
                    <div class="favorite-image">
                        ...
                        <button class="remove-fav-btn" onclick="toggleFavorite({{ $trip->id }})" title="{{ __('Remove from favorites') }}">
                            <i class="fas fa-heart"></i>
                        </button>
                    </div>

                    <div class="favorite-body">
                        <a href="{{ route('trips.show', $trip->id) }}" class="fav-trip-title" target="_blank">
                            {{ $trip->title }}
                        </a>
                        ...
                        <div class="fav-footer">
                            ...
                            <a href="{{ route('trips.show', $trip->id) }}" class="btn-view-trip" target="_blank">
                                {{ __('View Trip') }} <i class="fas fa-external-link-alt ms-1" style="font-size:0.75rem;"></i>
                            </a>
                        </div>
                    </div>
                </div>
...
function toggleFavorite(tripId) {
    fetch(`{{ url('agent/favorites') }}/${tripId}/toggle`, {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (!data.error) {
            ...
            const container = document.getElementById(`fav-container-${tripId}`);
