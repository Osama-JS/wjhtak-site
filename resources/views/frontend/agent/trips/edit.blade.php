    ...
    <form action="{{ route('agent.trips.update', $trip->id) }}" method="POST">
        ...
        <div class="form-section-card">
            <div class="card-header-premium">
                ...
                <h5>{{ __('Modify Trip Information') }}</h5>
            </div>
            <div class="card-body-premium">
                <div class="premium-row">
                    <div>
                        <label class="label-premium">{{ __('Trip Title') }}</label>
                        <input type="text" name="title" class="input-premium" value="{{ old('title', $trip->title) }}" required>
                    </div>
                    <div>
                ...
                <div class="form-group-premium">
                    ...
                    <textarea id="description" name="description" class="input-premium">{{ old('description', $trip->description) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="card-body-premium">
                ...
                <div class="premium-row-3">
                    <div>
                        ...
                        <input type="number" name="price" class="input-premium" value="{{ old('price', $trip->price) }}" required>
                    </div>
                    <div>
                        <label class="label-premium">{{ __('Original Price') }} (SAR)</label>
                        <input type="number" name="price_before_discount" class="input-premium" value="{{ old('price_before_discount', $trip->price_before_discount) }}">
                    </div>
                    <div>
                        <label class="label-premium">{{ __('Tickets Info') }}</label>
                        <input type="text" name="tickets" class="input-premium" value="{{ old('tickets', $trip->tickets) }}" placeholder="{{ __('e.g. Economy Class') }}">
                    </div>
                </div>
                ...
                <div class="premium-row-3">
                    <div>
                        ...
                        <input type="number" name="personnel_capacity" class="input-premium" value="{{ old('personnel_capacity', $trip->personnel_capacity) }}">
                    </div>
                    <div>
                        <label class="label-premium">{{ __('Base Capacity') }}</label>
                        <input type="number" name="base_capacity" class="input-premium" value="{{ old('base_capacity', $trip->base_capacity) }}">
                    </div>
                    <div>
                        <label class="label-premium">{{ __('Extra Pax Price') }}</label>
                        <input type="number" name="extra_passenger_price" class="input-premium" value="{{ old('extra_passenger_price', $trip->extra_passenger_price) }}">
                    </div>
                </div>
                ...
                <div class="premium-row">
                    <div>
                        ...
                        <input type="text" name="duration" class="input-premium" value="{{ old('duration', $trip->duration) }}" placeholder="{{ __('e.g. 5 Days') }}">
                    </div>
                    <div>
                        <label class="label-premium">{{ __('Expiry Date') }}</label>
                        <input type="date" name="expiry_date" class="input-premium" value="{{ old('expiry_date', $trip->expiry_date?->format('Y-m-d')) }}" required>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="form-actions-premium">
            ...
            <button type="submit" class="btn-premium btn-save-premium">
